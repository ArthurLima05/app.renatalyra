...
function buildOdontogramSvg(): string {
  ...
  const DH = 9,  DC = 4;     // dente decíduo — levemente menor que permanente
  const CX = 280;             // centro X do SVG
  const S = 23, DS = 20;     // step permanente / decíduo

  const pg = 12; // gap do centro ao centro do dente 11/21/41/31
  const dg = 11; // gap do centro ao centro do dente 51/61/81/71
  ...
  // Apenas o losango + quadrado central (sem número)
  const shape = (cx: number, cy: number, h: number, c: number) =>
  ...
  // Apenas o número (renderizado depois de todas as formas — sempre visível)
  const lbl = (cx: number, numY: number, num: number) =>
    `<text x="${cx}" y="${numY}" text-anchor="middle" font-size="7" fill="#1a1a1a" font-family="Arial,sans-serif">${num}</text>`;

  // Y dos centros e posições dos números
  const YUP = 23,  YUP_N = 10;   // permanentes superiores — número acima do dente
  const YUDEC = 47, YUDEC_N = 36; // decíduos superiores — número entre fileira perm e dec
  const YLDEC = 76, YLDEC_N = 65; // decíduos inferiores — número entre fileira dec e dec (acima do dente)
  const YLP = 100,  YLP_N = 116;  // permanentes inferiores — número abaixo do dente
  ...
  // ── FASE 1: todas as formas primeiro ──────────────────────────────────────────

  // Permanentes superiores
  for (let i = 1; i <= 8; i++) {
    s += shape(xP(i),  YUP,   H,  C);
    s += shape(xPL(i), YUP,   H,  C);
  }
  // Decíduos superiores
  for (let i = 1; i <= 5; i++) {
    s += shape(xD(i),  YUDEC, DH, DC);
    s += shape(xDL(i), YUDEC, DH, DC);
  }
  // Decíduos inferiores
  for (let i = 1; i <= 5; i++) {
    s += shape(xD(i),  YLDEC, DH, DC);
    s += shape(xDL(i), YLDEC, DH, DC);
  }
  // Permanentes inferiores
  for (let i = 1; i <= 8; i++) {
    s += shape(xP(i),  YLP,   H,  C);
    s += shape(xPL(i), YLP,   H,  C);
  }

  // ── FASE 2: todos os números por cima (nunca cobertos por formas) ─────────────

  // Permanentes superiores
  for (let i = 1; i <= 8; i++) {
    s += lbl(xP(i),  YUP_N,   10 + i);
    s += lbl(xPL(i), YUP_N,   20 + i);
  }
  // Decíduos superiores
  for (let i = 1; i <= 5; i++) {
    s += lbl(xD(i),  YUDEC_N, 50 + i);
    s += lbl(xDL(i), YUDEC_N, 60 + i);
  }
  // Decíduos inferiores
  for (let i = 1; i <= 5; i++) {
    s += lbl(xD(i),  YLDEC_N, 80 + i);
    s += lbl(xDL(i), YLDEC_N, 70 + i);
  }
  // Permanentes inferiores
  for (let i = 1; i <= 8; i++) {
    s += lbl(xP(i),  YLP_N,   40 + i);
    s += lbl(xPL(i), YLP_N,   30 + i);
  }
...
export function generateFichaOdontologicaHtml(params: {
  ...
  const qRows = activeQ.map((q, i) => {
    ...
    const qualLine  = needsQual
      ? `&nbsp;&nbsp;QUAL?&nbsp;<span style="display:inline-block;min-width:120px;border-bottom:1px solid #555;font-size:10.5px">${qualText}</span>`
      : "";
  ...
  const legendHtml = LEGEND.map(
    (l) =>
      ...
      `<span style="font-size:10px">${l.label}</span></span>`,
  ).join("");
  ...
  const fld = (lbl: string, val: string, w = "auto") =>
    ...
    `<span style="font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#777">${lbl}</span>` +
    `<span style="border-bottom:1px solid #333;min-height:15px;font-size:12px;padding-bottom:1px">${val}</span></div>`;
  ...
  const css = `
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    @media print{
      body{padding:7mm 11mm;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      .print-bar{display:none!important}
    }
    .header{display:flex;align-items:center;padding-bottom:6px;border-bottom:2.5px solid #c8a84b;margin-bottom:7px}
    .logo{height:42px;object-fit:contain;max-width:145px;flex-shrink:0}
    .h-title{flex:1;text-align:center;font-size:15px;font-weight:700;letter-spacing:.05em;line-height:1.3}
    .logo-spacer{width:145px;flex-shrink:0}
    .data-row{display:flex;gap:0;margin-bottom:4px;align-items:flex-end}
    .sec{font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#c8a84b;margin:7px 0 4px;padding-bottom:2px;border-bottom:1px solid #e8dfc8}
    .odonto-box{border:1px solid #ddd;border-radius:4px;padding:5px 8px 4px;margin-bottom:4px}
    table.qt-tbl{width:100%;border-collapse:collapse;font-size:12px}
    table.qt-tbl tr{border-bottom:1px dotted #ddd}
    table.qt-tbl td{padding:2px 4px;vertical-align:top;line-height:1.3}
    td.qn{width:20px;color:#888;font-weight:600;white-space:nowrap;vertical-align:top}
    td.qt{line-height:1.3}
    td.qa{white-space:nowrap;text-align:right;font-size:11.5px;vertical-align:top}
    .auth{font-size:10.5px;text-align:center;font-style:italic;margin:8px 0 8px;color:#333;line-height:1.4}
    .sig-row{display:flex;gap:16px;margin-top:6px}
    .sig-box{flex:1;text-align:center}
    .sig-line{border-top:1px solid #1a1a1a;padding-top:4px;font-size:10.5px;color:#555;margin-top:22px}
...
<body>
  ...
  <div class="data-row">
    ...
    <div style="display:flex;flex-direction:column;flex-shrink:0">
      <span style="font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#777">Sexo</span>
      <span style="border-bottom:1px solid #333;min-height:15px;font-size:12px;padding-bottom:1px;white-space:nowrap">
  ...
  <div class="odonto-box">
    ${buildOdontogramSvg()}
    <div style="display:flex;flex-wrap:wrap;margin-top:5px;gap:2px 0">${legendHtml}</div>
  </div>
