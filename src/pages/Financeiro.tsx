...
export default function Financeiro() {
  const { transactions, addTransaction, deleteTransaction, installments, updateInstallment, sessions, getPatientById } =
    useClinic();
  ...
  const categoriesData = filteredByDateTransactions.reduce(
    (acc, t) => {
      if (!acc[t.category]) {
  ...
  const evolutionData = (() => {
    ...
    return last6Months.map((month) => {
      ...
      const entrada = monthTransactions.filter((t) => t.type === "entrada").reduce((acc, t) => acc + t.amount, 0);

      const saida = monthTransactions.filter((t) => t.type === "saida").reduce((acc, t) => acc + t.amount, 0);
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setTransactionData({ ...transactionData, type: value as TransactionType })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Select
                        value={dateOption}
                        onValueChange={(value) => setDateOption(value as "hoje" | "personalizado")}
                      >
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar
                              ...
                              modifiersStyles={{
                                selected: {
                                  backgroundColor: "#DBC192", // cor personalizada
                                  color: "#000000", // texto preto para contraste
                                  fontWeight: "600",
                                  borderRadius: "0.5rem", // arredondamento da célula
                                },
                              }}
                            />
                          </PopoverContent>
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1 pr-2">
                  ...
                  <h3
                    className={`text-xl sm:text-2xl font-bold truncate ${saldo >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    R$ {saldo.toFixed(2)}
                  </h3>
          ...
          <CardContent>
            <Tabs defaultValue="distribuicao" className="w-full">
              <TabsList className="grid w-full grid-cols-4 h-auto">
                <TabsTrigger value="distribuicao" className="text-xs sm:text-sm">
                  Distribuição
                </TabsTrigger>
                ...
                <TabsTrigger value="previsoes" className="text-xs sm:text-sm">
                  Previsões
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="previsoes" className="mt-4">
                <div className="space-y-4">
                  ...
                  {installments.filter((i) => !i.paid && i.sessionId).length === 0 ? (
                    <p className="text-center text-muted-foreground py-8">Nenhuma parcela futura cadastrada</p>
                  ) : (
                    <div className="overflow-x-auto">
                            ...
                            .map((installment) => {
                              ...
                              return (
                                <TableRow key={installment.id}>
                                  ...
                                  <TableCell>{session?.type || "Sessão não encontrada"}</TableCell>
                                  <TableCell>
                                  ...
                                  <TableCell className="text-right">
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        updateInstallment(installment.id, { paid: true, paidDate: new Date() })
                                      }
                                    >
                      ...
                      {installments.filter((i) => !i.paid && i.sessionId).length > 8 && (
                        <div className="mt-4 text-center">
                          <Button variant="outline" onClick={() => setShowAllInstallments(!showAllInstallments)}>
                            {showAllInstallments
                              ? "Mostrar menos"
                              : `Exibir mais (${installments.filter((i) => !i.paid && i.sessionId).length - 8} parcelas)`}
                          </Button>
                        </div>
                      )}
              ...
              {filteredTransactions.length === 0 ? (
                ...
                filteredTransactions
                  ...
                  .map((transaction) => (
                    <div
                      key={transaction.id}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 border rounded-lg hover:bg-secondary/50 transition-colors gap-3"
                    >
                      ...
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        <div className="text-right flex-1 sm:flex-shrink-0">
                          <p
                            className={`font-bold text-base sm:text-lg ${transaction.type === "entrada" ? "text-green-600" : "text-red-600"}`}
                          >
