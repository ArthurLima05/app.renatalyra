...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const webhookUrl = Deno.env.get('N8N_FALTA_NOTIFICATION_WEBHOOK_URL')
    if (!webhookUrl) {
      return new Response(
        JSON.stringify({ error: 'Webhook de falta não configurado no servidor' }),
    ...
    if (!n8nRes.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Falha ao acionar notificação de falta (${n8nRes.status})` }),
    ...
    console.error('Erro em trigger-falta-notification:', error)
    return new Response(
